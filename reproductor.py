...
class DialogoReproductor(wx.Dialog):
    ...
    def _preparar_hilo(self):
        ...
        except ErrorVideoNoDisponible as exc:
            wx.CallAfter(self._error_video_no_disponible, str(exc))
        except ErrorReproductor as exc:
            wx.CallAfter(self._error_falta_mpv, str(exc))
        except Exception as exc:
    ...
    def _error_youtube_bloqueo(self, mensaje):
        self.preparando = False
        self.estado.SetValue(traducir_dinamico("YouTube requiere verificación para reproducir este video."))
        hablar_async("YouTube requiere verificación para reproducir este video", limpiar=True)
        wx.MessageBox(
    ...
    def _error_video_no_disponible(self, mensaje):
        self.preparando = False
        self.estado.SetValue(traducir_dinamico("Este video no está disponible."))
        hablar_async("Este video no está disponible", limpiar=True)
        wx.MessageBox(
            traducir_clave("__player.video_unavailable.body__").format(detalle=mensaje),
            "Reproductor interno",
            wx.OK | wx.ICON_INFORMATION,
        )
        self._detener_y_cerrar(anunciar=False)

    def _error_falta_mpv(self, mensaje):
        self.preparando = False
        self.estado.SetValue(traducir_dinamico("Falta el componente MPV necesario para reproducir."))
        hablar_async("Falta el componente MPV necesario para reproducir", limpiar=True)
        wx.MessageBox(
            traducir_clave("__player.mpv_missing.body__").format(detalle=mensaje),
            "Reproductor interno",
            wx.OK | wx.ICON_ERROR,
        )
        self._detener_y_cerrar(anunciar=False)
