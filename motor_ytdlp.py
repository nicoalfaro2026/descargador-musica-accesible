...
def normalizar_canal_motor(canal):
    canal = str(canal or "stable").strip().lower()
    return canal if canal in CANALES_MOTOR else "stable"


def repositorio_canal_motor(canal="stable"):
    return CANALES_MOTOR[normalizar_canal_motor(canal)]


def url_descarga_motor(canal="stable"):
    repo = repositorio_canal_motor(canal)
    return f"https://github.com/{repo}/releases/latest/download/{NOMBRE_MOTOR}"
...
def actualizar_motor_descarga(callback=None, canal="stable"):
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _descargar_archivo(url_descarga_motor(canal), tmp, callback=callback)
...
def version_mas_reciente_disponible(canal="stable", timeout=15):
    """Consulta la versión más reciente publicada del canal elegido de yt-dlp.

    Canales oficiales: stable, nightly y master.
    Devuelve la etiqueta de versión o None si no se pudo consultar.
    """
    try:
        import json
        canal = normalizar_canal_motor(canal)
